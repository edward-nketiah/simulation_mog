function[data,lambda, k, log_kikelihood] = di_mog2(space_time_data,P1,n1,k1,k2,b_data,o_data,p_b, mu_b, var_b,p_o, mu_o, var_o)
x = b_data(:,1);
y = b_data(:,2);
x2 = o_data(:,1);
y2 = o_data(:,2);
...
%k= lambda/
%k = (n_data*n_b + n_o)/(n_data*4*7*145.54); %phelt-145.54, burglary-*8*15*244.2083
%k = n_data/(20*20*1350);
%k = n_data/(20*20*1350);
temp = (60* lambda.^-1 ./ sum(lambda.^-1)) > rand(n_data,1 );
